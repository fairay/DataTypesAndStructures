...
aqueue_t create_aque(void)
{
    ...
    code = create_array(&que.arr, que.size);
...
{
    int code = resize_array(&que->arr, new_size);
...
int decrease_aque(aqueue_t *que, int new_size)
{
    ...
    else if (que->pin >= new_size)
    {
        for (int i = 0; i < que->count; i++)
            que->arr[i] = que->arr[que->pout + i];
        que->pout = 0;
        que->pin = que->count;
    }
    code = resize_array(&que->arr, new_size);
    que->size = new_size;
    return code;
}
